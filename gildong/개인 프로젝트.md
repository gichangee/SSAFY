...
2. 설정파일
    ...
    ![image](https://github.com/gichangee/SSAFY/blob/master/gildong/image.png)
      ...
      data:
        web:
          pageable:
            ...
            //웹 요청을 통해서 넘어왔을 때만 자동적으로 보정을 해줌
            //수동으로 할땐 적용안됨
            //false면 0부터 시작 true면 1부터 시작
    ```
...
2. 컨트롤러
    ...
    public class PostController {
        ...
        //public List<PostResponse> getList(@PageableDefault Pageable pageable){ -> @PageableDefault의 디폴트 값이 10개임
        //public List<PostResponse> getList(@PageableDefault(size = 5) Pageable pageable){ 이렇게 하거나 설정파일 변경하거나
        @GetMapping("/posts")
...
3. 서비스
    ...
    public class PostService {
        ...
        // 글이 1,000,000,000,000개 있을경우 -> DB글 모두 조회하는 경우 -> DB가 뻗을 수 있다
        // DB -> 애플리케이션 서버로 전달되는 시간, 트래픽비용이 많이 발생할 수 있따
        // 글 전체를 조회하는 경우는 별로 없다
        
        //import org.springframework.data.domain.Pageable; 이거 불러와야함
    
        public List<PostResponse> getList(Pageable pageable) {
            //int page를 매개변수로 받을 때
...
4. 테스트
    ...
    class PostControllerTest {
        ...
        void test5() throws Exception {
            ...
            // json응답에서 title값 길이를 최대 10글자로 해주세요
            //expected
            //매번 size 값 넣어주기 귀찮음 -> yml수정 default-page-size: 5
            //size값을 넣으면 size값이 우선 /posts?page=1&sort=id,desc&size=10
...
# 11. 게시글 조회 5 -페이징 처리(QueryDSL)

![image.png](%E1%84%80%E1%85%A2%E1%84%8B%E1%85%B5%E1%86%AB%20%E1%84%91%E1%85%B3%E1%84%85%E1%85%A9%E1%84%8C%E1%85%A6%E1%86%A8%E1%84%90%E1%85%B3%20b73bed5326ff4dfcac3147a56ba11b51/image%201.png)

![image.png](%E1%84%80%E1%85%A2%E1%84%8B%E1%85%B5%E1%86%AB%20%E1%84%91%E1%85%B3%E1%84%85%E1%85%A9%E1%84%8C%E1%85%A6%E1%86%A8%E1%84%90%E1%85%B3%20b73bed5326ff4dfcac3147a56ba11b51/image%202.png)
...
1. 컨트롤러 
    ...
    public class PostController {
        ...
        /*
        @RequestParam
        단일 값의 요청 파라미터를 간단하게 메서드에 바인딩하고 싶을 때 사용합니다.
        /posts?page=1&size=10 같은 요청이 있을 때, page와 size 변수에 각각 1과 10이 바인딩됩니다.
        @RequestParam의 기본 동작은 해당 파라미터가 필수임을 의미합니다.
    
        @GetMapping("/posts")
        public List<PostResponse> getList(@RequestParam int page, @RequestParam int size) {}
    
        @ModelAttribute
        요청 파라미터들을 객체에 바인딩하고, 그 객체를 메서드의 파라미터로 전달할 때 사용됩니다. 주로 폼 데이터를 객체로 묶을 때 사용합니다.
        여러 개의 요청 파라미터를 한 번에 객체로 묶어야 하는 경우 유용합니다.
    
        /posts?page=1&size=10이라는 요청이 오면 PostSearch 객체의 page와 size 필드가 각각 1과 10으로 설정됩니다.
        @GetMapping("/posts")
...
4. request DTO
    ...
    /*
    클래스에 @Builder를 붙이는 경우: 해당 클래스의 모든 필드에 대한 빌더 패턴이 제공됩니다.
    클래스 전체에 대해 빌더 패턴을 사용하고 싶을 때 유용합니다.
    생성자에 @Builder를 붙이는 경우: 특정 생성자에 대해서만 빌더 패턴이 제공됩니다.
    생성자의 매개변수로 전달되는 필드들에 대해 빌더 패턴을 사용하고 싶을 때 유용합니다.
    필요한 필드만으로 객체를 생성하고자 할 때 사용됩니다.
     */
    ```
